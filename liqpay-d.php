...
class liqpay_donate_saphali {
	...
	function __construct() {
		load_plugin_textdomain( 'saphali-liqpay-for-donate',  false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	...
	function adminMenu() {
		if ( function_exists('add_menu_page') ) {
			$this->menu_id = add_menu_page( __('LiqPay','saphali-liqpay-for-donate'), __('LiqPay','saphali-liqpay-for-donate'), 'manage_options' ,'liqpay-config', array($this,'configPage') , plugins_url('images/menu_icons.png', __FILE__));
		}
	}
	function configPage () {
		...
		if( sanitize_text_field( $_POST['liqpay_privat_key'] ) !=  $_POST['liqpay_privat_key'] ) {
			$error = new WP_Error ('broke', __( "Секретный ключ не сохранен, т.к. имеет запрещенные символы", 'saphali-liqpay-for-donate' ) );
		...
		?>
		<h3><?php _e('LiqPay - Настройка','saphali-liqpay-for-donate'); ?></h3>
   ...
   echo '<div id="message" class="error"><p>' . $error_string . '</p></div>';
		...
		<table class="wp-list-table widefat filds_comment">
			...
			<tr><th width="120px"><?php _e('Публичный ключ', 'saphali-liqpay-for-donate') ; ?></th><td><input  type="text" name="liqpay_public_key" value='<?php echo $public_key; ?>' /></td></tr>
			<tr><th><?php _e('Приватный ключ', 'saphali-liqpay-for-donate'); ?></th><td><input type="password" name="liqpay_privat_key" size="60" value='<?php echo $privat_key; ?>' /></td></tr>
			</tbody></table>
		<div class="clear"></div>
		<p><button class="button-primary"><?php _e('Сохранить','saphali-liqpay-for-donate'); ?></button></p>
		</form>
		<?php
	}
		...
		<script type="text/javascript" charset="utf-8">
			jQuery(document).ready(function(){
				if(typeof(QTags) !== 'undefined') {
					QTags.addButton( 'saphali_liqpay', '<?php _e('Добавить кнопку пожертвований', 'saphali-liqpay-for-donate'); ?>', '[saphali_liqpay amount="10" desc="<?php _e('Пожертвование', 'saphali-liqpay-for-donate'); ?>"]');
				}
			});
		</script>
		<?php 
